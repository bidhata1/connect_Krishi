...
class ProductDatabaseHelper {
  ...
  Stream<List<Review>> getAllReviewsStreamForProductId(
    ...
    final List<Review> reviews = []; // Use modern list literal syntax
  ...
  Future<List<String>> getCategoryProductsList(ProductType productType) async {
    ...
    // Use the map() method for a more concise transformation
    final productsId = querySnapshot.docs.map((product) => product.id).toList();
    return productsId;
  }
  ...
  Future<List<String>> get usersProductsList async {
    ...
    // Use the map() method for a more concise transformation
    final usersProducts = querySnapshot.docs.map((doc) => doc.id).toList();
  ...
  Future<List<String>> get allProductsList async {
    ...
    // Use the map() method for a more concise transformation
    final productsId = productsQuerySnapshot.docs.map((doc) => doc.id).toList();
    return productsId;
  }
